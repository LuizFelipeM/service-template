	...
	app.use('*all', async (req, res) => {
		try {
			...
			if (typeof render !== 'function')
				throw Error(
					`Unable to call render, expect type function but received ${typeof render}`,
				)
...
async function getTemplateAndRender(
	vite: ViteDevServer,
	url: string,
): Promise<{ template: string; render: unknown }> {
	if (!isProduction) {
		// Always read fresh template in development
		const index = await fs.readFile(
			'./src/presentation/pages/index.html',
			'utf-8',
		)
		const template = await vite.transformIndexHtml(url, index)
		const render = (
			await vite.ssrLoadModule('/src/presentation/pages/entry-server.tsx')
		).render
		...
		return {
			template,
			// @ts-expect-error File only provided on build
